...
const onnx_runner = {
    ...
    tiled_render: async function (image_data, config, alpha_config,
        ...
        // setup output via OffscreenCanvas (Worker 환경에서 DOM Canvas 대신 사용)
...
(function () {
    ...
    WAIFU2X = async (image_data, options) => {
        ...
        const has_alpha = alpha_enabled ? onnx_runner.check_alpha_channel(image_data.data) : false;
        const Alpha = has_alpha && options.alpha_config ? options.alpha_config.split(",") : null;
        const alpha_config = Alpha ? CONFIG.get_config(Alpha[0], Alpha[1], Alpha[2]) : null;

        const resultImageData = await onnx_runner.tiled_render(
...
onmessage = async function (e) {
    ...
    if (data.type === "process") {
        try {
            const resultImageData = await WAIFU2X(data.imageData, data.options);
            // ImageData의 buffer를 Transferable로 반환
            postMessage({
                type: "result",
            ...
            console.error("waifu2x worker error:", err);
            postMessage({
                type: "error",
                frameIndex: data.frameIndex,
                error: err.message
    ...
    // 레거시 호환: type 없는 메시지 (기존 방식)
    try {
        const result = await WAIFU2X(data.data || data.imageData, data.options);
        postMessage(result);
    } catch (err) {
        console.error("waifu2x worker error (legacy):", err);
    }
};